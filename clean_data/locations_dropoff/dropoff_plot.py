...
p1 = pd.read_csv("dropoff-2019-01.csv.csv")
p2 = pd.read_csv("dropoff-2019-02.csv.csv")
p3 = pd.read_csv("dropoff-2019-03.csv.csv")
p4 = pd.read_csv("dropoff-2019-04.csv.csv")
p5 = pd.read_csv("dropoff-2019-05.csv.csv")
p6 = pd.read_csv("dropoff-2019-06.csv.csv")

p_all = pd.merge(left=p1,
	right=p2,
	right_on=["DOLocationID"],
	left_on=["DOLocationID"]).rename({"Number of DropOffs_x":"01", "Number of DropOffs_y":"02"}, axis=1)
p_all = p_all.merge(p3,
	right_on=["DOLocationID"],
	left_on=["DOLocationID"])
p_all = p_all.merge(p4,
	right_on=["DOLocationID"],
	left_on=["DOLocationID"]).rename({"Number of DropOffs_x":"03", "Number of DropOffs_y":"04"}, axis=1)
p_all = p_all.merge(p5,
	right_on=["DOLocationID"],
	left_on=["DOLocationID"])
p_all = p_all.merge(p6,
	right_on=["DOLocationID"],
	left_on=["DOLocationID"]).rename({"Number of DropOffs_x":"05", "Number of DropOffs_y":"06"}, axis=1)
p_all["Half Year Sum"]=p_all["01"]+p_all["02"]+p_all["03"]+p_all["04"]+p_all["05"]+p_all["06"]
#print(p_all)
#p_all.to_csv("dropoff-all.csv")
...
plt.title("Number of Drop offs at Different Locations (2019 Jan. to Jun.)")
